...
document.addEventListener('DOMContentLoaded', function() {
    const form = document.getElementById('contact-form-01');
    ...
    form.addEventListener('submit', function(event) {
        ...
        fetch(form.action, {
            ...
            if (response.ok) {
                ...
                formResults.innerHTML = '<strong>Thank you!</strong> Your message has been sent successfully.';
                form.reset(); 
            } else {
                formResults.style.display = 'block';
                formResults.innerHTML = '<strong>Oops!</strong> There was a problem sending your message.';
            }
        }).catch(error => {
            formResults.style.display = 'block';
            formResults.innerHTML = '<strong>Error!</strong> Unable to send message.';
        });
    });
});
